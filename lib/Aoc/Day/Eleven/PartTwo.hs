module Aoc.Day.Eleven.PartTwo (solvePart2) where
...
monkeyWorry :: Int -> Monkey -> Int -> Int
monkeyWorry modp mokey = (`mod` modp) . (performOperation (monkeyOperation mokey))
...
monkeyVerdict :: Int -> Monkey -> Int -> (Int, Int)
monkeyVerdict modp mokey x = (new_target, new_worry)
  where new_worry = monkeyWorry modp mokey x
        new_target = monkeyTarget mokey new_worry

monkeyDenom :: [Monkey] -> Int
monkeyDenom monkeys = foldl1 lcm tests
  where tests = map monkeyTest monkeys
...
monkeyPass monkey_idx = do
  ...
  let curr_monkey = getMonkey monkey_idx monkeys
      common = monkeyDenom monkeys
      (Holding curr_items) = monkeyStack curr_monkey
      item_destins = map (monkeyVerdict common curr_monkey) curr_items
...
solvePart2 :: Text -> Int
solvePart2 in_text = evalState (monkeyFun 10000) monkeys
